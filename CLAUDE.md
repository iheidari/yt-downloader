...
**Pending/failed rows (client-only lifecycle):** downloads run server-side and finish even if the user leaves the download page, so history rows are written **at click time**, not on SSE completion. `handleDownload` (`InfoPage.jsx`) calls `startPending()` to write a `status: 'downloading'` row to `tubekeepHistory` immediately (fields: `downloadId`, `url`, `type`, `title`, `thumbnail`, `createdAt` — no `filename`/`size` yet). On SSE `complete`, `addDownload(data.data)` replaces it in place with the full completed record (which carries **no** `status`), so it renders as a normal `ActiveCard`. On SSE `error`, `markFailed()` flips it to `status: 'failed'`. `DownloadsPage` routes `status === 'downloading'` → `DownloadingCard` (spinner, links to `/download/:id`) and `status === 'failed'` → `FailedCard` (Redownload + Dismiss) **before** the `ActiveCard` fallback. Because these rows aren't on the server yet, `sync()` preserves local `downloading`/`failed` rows whose `downloadId` isn't in the server's id set (mirroring `preservedMoved`), and Dismiss uses `dropLocal()` — a local-only removal with **no** server `DELETE` and no move to the expired list (unlike `removeDownload`).

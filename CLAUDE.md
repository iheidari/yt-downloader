...
2. `POST /api/download` → mints a `downloadId` (a UUID) **and starts the download job** server-side via the download manager. The parameters ride the **request body** (`url`, `formatId`, `type`, `title`, `thumbnail`, `keep`, `filesize`). The job runs yt-dlp to completion **independent of any client connection** — navigating away no longer aborts it. Three guards run here, before any job starts: over the concurrency cap it returns **HTTP 429**; if the client-passed `filesize` would push the user past their **per-user quota** (`hasQuotaFor`) it returns **HTTP 507**; and if it fails the **global disk** fit margin (`hasRoomFor`) it returns **HTTP 507** too — all `{ success: false, error }` the UI surfaces inline. Once past the guards the download is **recorded in the `downloads` table** under the requesting user (as `status: 'downloading'`) *before* the job starts, so an in-flight download survives a reload; the job's terminal hooks flip that row to `complete` (writing the real filename + on-disk size) or `failed`.
...
Both live in `utils/storage.js` and the frontend reads the same knobs + quota numbers back out of the `/api/disk` response (`hasRoomFor` / `hasQuotaFor` / `downloadBlockReason` in `lib/media.js`), so client disable-check and server backstop can't drift. `FormatSelector` shows a **"Your storage"** banner (used / left, or "unlimited") and disables any format either guard rejects, with the reason under it. Unknown-size formats are never blocked by either. The client-passed `filesize` is untrusted (a UX guard, not a security boundary) — the row's size is corrected to the real one on completion. The two guards fail differently on error: the quota check **fails closed** (a DB outage must not hand out free storage), the disk check **fails open**.

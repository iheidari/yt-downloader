...
Use the WebFetch tool to gather page data. **Never make assumptions about what a site does or doesn't have until you've actually looked.** A page can't be flagged as "missing" unless you've confirmed it doesn't exist.
...
Immediately after the brief chat recap, generate the full report as a `.docx` (and a `.pdf` if a converter is available — see "Convert to PDF"). Do not ask the user if they want this — just produce it.

Tell the user: "Generating your downloadable report now..."

### Output location

Write reports into an `seo-audits/` directory inside the current working directory (create it if needed). Use a filename like `seo-audit-example-com-2026-06-13.docx` (domain with hyphens, ISO date). All commands below use **relative paths from the current working directory** — substitute the real domain/date.
...
If the global install fails (permissions), fall back to a local install in a scratch dir: `mkdir -p ~/.claude/skills/seo-geo-aeo/.runtime && cd ~/.claude/skills/seo-geo-aeo/.runtime && npm install docx`, then run the report script with `NODE_PATH=~/.claude/skills/seo-geo-aeo/.runtime/node_modules node report.js`.

**Then immediately write and run the full report script in the next tool call — do not pause, do not add intermediate steps.** Write the complete JS to a file (e.g. `seo-audits/report.js`) and execute it in one shot.
...
- Line 1: Audit date
- Line 2: "Generated with Claude Code — SEO/GEO/AEO audit skill"
...
**Footer:** "Generated with Claude Code" left-aligned, page number right-aligned. Separated by a gray top border.
...
PDF conversion needs LibreOffice. Try it; if `soffice` isn't installed, skip the PDF and tell the user how to get it.
...
Report the file paths with clickable markdown links (relative to the working directory):

```
Your audit report is ready:
- [Download Word Doc](seo-audits/seo-audit-[domain]-[date].docx)
- [Download PDF](seo-audits/seo-audit-[domain]-[date].pdf)   ← only if the PDF was generated
```

If only the DOCX was produced, link just that and note the PDF instructions above.
